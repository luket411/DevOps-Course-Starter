...
def test_create_item(param, attribute, value):
    ...
    assert item.__getattribute__(
        attribute) == value, f"Item.{attribute} not set to {value} correctly"


@pytest.mark.parametrize("attribute, expected_value, card_changes, list_changes",
                         [
                             ("name", "some_test_card_name", dict(
                                 name="some_test_card_name"), dict()),
                             ("trello_id", "some_test_id", dict(
                                 id="some_test_id"), dict()),
                             ("id", "some_test_short_id", dict(
                                 idShort="some_test_short_id"), dict()),
                             ("status", "some_test_list_name", dict(),
                              dict(name="some_test_list_name"))
                         ])
def test_from_trello_card(attribute, expected_value, card_changes, list_changes):
    mock_card = dict(name="mock_card_name", id="mock_id",
                     idShort="mock_id_short")
